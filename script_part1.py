...
#name_territory_full = "Parc de la Sainte-Baume" # possible name = ["Parc de la Sainte-Baume","Parc du Luberon","PNR Baronnies provençales","Parc des Alpilles","Parc du Mont-Ventoux","Parc du Queyras","Parc du Verdon","Parcs des Monts d'Ardèche","Parc de Corse"]
...
# photo = pd.read_csv(f"data_input/Image/flickr_location_{name_territory}.csv") #File where the informations about the photo are located
# image_caracteristic = pd.read_csv(f"data_input/Image/{name_territory}_inference.csv") # Sainte-Baume_inference.csv
...
# territory_L93 = parcs.loc[parcs["short_name"] == name_territory].to_crs(2154) #Keeping only one park
...
#list_dep_vegetal = [vegetal_04,vegetal_05,vegetal_06,vegetal_07,vegetal_13,vegetal_26,vegetal_83,vegetal_84] # General territory of study
list_vegetal = []
...
# vegetaltemp = gpd.clip(vegetal, territory_L93) 
# vegetaltemp = vegetaltemp[vegetaltemp.geometry.geom_type.isin(["Polygon", "MultiPolygon"])]
# vegetaltemp = vegetaltemp.dissolve()
# vegetaltemp.to_file(f"data_output/vegetation_{name_territory}.shp")
...
# grid.to_file(f"data_output/blank_square_{name_territory}.shp")
# grid = gpd.read_file(f"data_output/blank_square_{name_territory}.shp")
...
#urban_area_gdf.to_file(f"data_output/urban_area_{name_territory}.shp")
#urban_area_gdf = gpd.read_file(f"data_output/urban_area_{name_territory}.shp")
# #%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
center_town.to_file(f"data_output/centre_ville_{name_territory}.shp")
center_town = gpd.read_file(f"data_output/centre_ville_{name_territory}.shp")
...
# grid.to_file(f"data_output/season_{name_territory}.shp")
# grid = gpd.read_file(f"data_output/season_{name_territory}.shp")
...
dist = dist.drop(columns=["geometry",'u','v','key','osmid','maxspeed','est_width','access','junction','bridge','width','service','tunnel','highway','name','ref','oneway','reversed','length','lanes'],errors="ignore")
...
dist_road = dist_road.drop(columns=["geometry",'u','v','key','osmid','est_width','maxspeed','access','junction','bridge','width','service','tunnel','highway','name','ref','oneway','reversed','length','lanes'],errors="ignore")
# removing the columns not used
...
grid.to_file(f"data_output/distance_route_mairie_{name_territory}.shp")
...
# grid = grid.merge(grid_centroid_alti, on=["square_id",'sq_id_2X','nb_spring','nb_summer','nb_winter','nb_autumn','nat_autumn', 'nat_spring', 'nat_summer', 'nat_winter','leas_autum', 'leas_sprin', 'leas_sumer', 'leas_wint','nature','leasure'],how="left") #Inserting the value of elevation into the main data file
